...
function addToDictionary(dictionaryName, value) {
	let	requestParams = {"operation": "add", "value": value};
	return getRequest(dictionaryName, requestParams)
	.then(function(jsonResponse) {
		refreshDictionary(dictionaryName);		
	});	
}
...
function changeEmployeeInfo(employeeIdx) {
	setLoadingText("Изменение информации о сотруднике");
	let userId = employees[employeeIdx]["id"];
	getRequest("users/" + userId)
	.then(function(jsonResponse) {
		if (jsonResponse !== null) {
			let userJson = jsonResponse["user"];
			for (let checkResult of employees[employeeIdx]["checkResults"]) {
				if (checkResult["fieldName"] === undefined) {
					if (checkResult["id"] == null) {
						userJson["contacts"].push({"type": checkResult["fieldType"], "value": checkResult["newValue"]});
					} else {
						for (let contact of userJson["contacts"]) {
							if ((contact["type"] == checkResult["fieldType"]) && (contact["id"] == checkResult["id"])) {
								contact["value"] = checkResult["newValue"];
							}
						}	
					}
				} else {
					userJson[checkResult["fieldName"]] = checkResult["newValue"];	
				}				
			}
			requestParams = {"operation": "edit", "value": userJson};
			getRequest("users", requestParams)
			.then(function(jsonResponse) {				
				refreshEmployeeInfo(employeeIdx);
			});				
		} else {
			refreshEmployeeInfo(employeeIdx);	
			showMsgToast("danger", "Произошла ошибка при изменении информации о сотруднике!");	
		}		
	});
}
...
function loadEmployeesFile(fileEmpl, fileDict) {
	readFile(fileEmpl)
	.then(function(fileData) {
		employees = [];		
		let recs = fileData.split("\n"); 
		for (let idxRec=1; idxRec<recs.length; idxRec++) {
		  let newEmloyee = {"name": "", "lastName": "", "firstName": "", "middleName": "", "sex": "",
							"companyCode": "", "subdivisionCode": "", "jobName": "", "dateBegin": "", 
							"birthDate": "", "phone": "", "email": "", "lineNum": 1 + idxRec, "id": "",
							"internalUser": null, "checkResults": [], "status": "NEW"} 
		  let fields = recs[idxRec].split(";"); 
		  for (let idxField=0; idxField<fields.length; idxField++) {
			switch (idxField) {
			  case 0:
				newEmloyee["companyCode"] = trimStrAll(fields[idxField]);
				break;
			  case 1:
				newEmloyee["name"] = trimStrAll(fields[idxField]);
				let nameArr = newEmloyee["name"].split(" ");
				for (let idxNameArr=0; idxNameArr<nameArr.length; idxNameArr++) {	
					switch (idxNameArr) {
						case 0:
							newEmloyee["lastName"] = nameArr[idxNameArr];
							break;
						case 1:
							newEmloyee["firstName"] = nameArr[idxNameArr];
							break;
						case 2:
							newEmloyee["middleName"] = nameArr[idxNameArr];
							break;
						default:
							newEmloyee["middleName"] += " " + nameArr[idxNameArr];
							break;				
				...
				break;
			  case 2:
				newEmloyee["subdivisionCode"] = trimStrAll(fields[idxField]);
				break;			
			  case 7:
				newEmloyee["sex"] = "m";
				if (trimStrAll(fields[idxField]) != "Мужской") {
				  newEmloyee["sex"] = "w";
				}
				break;	
			  case 14:
				newEmloyee["dateBegin"] = formatDate(new Date(fields[14], fields[12] - 1, fields[13]), true);
				break;				
			  case 15:
				newEmloyee["jobName"] = trimStrAll(fields[idxField]);
				break;	
			  case 20:
				newEmloyee["birthDate"] = formatDate(new Date(fields[20], fields[19] - 1, fields[18]), true);
				break;
			}
		  }
		  if (newEmloyee["name"] != "") {
			employees.push(newEmloyee);	  
		  }
		}
		loadDictionaryFile(fileDict);
	});	
}

function loadDictionaryFile(fileDict) {
	readFile(fileDict)
	.then(function(fileData) {	
		let recs = fileData.split("\n"); 
		for (let idxRec=1; idxRec<recs.length; idxRec++) {
		  let newEmloyee = {"name": "", "location": "", "phone": "", "email": ""} 
		  let fields = recs[idxRec].split(";"); 
		  for (let idxField=0; idxField<fields.length; idxField+=1) {
			switch (idxField) {
			  case 1:
				newEmloyee["name"] = trimStrAll(fields[idxField]);		
				break;
			  case 2:
				newEmloyee["location"] = trimStrAll(fields[idxField]);
				break;							
			  case 3:
				newEmloyee["phone"] = trimStrAll(fields[idxField]);
				break;	
			  case 4:
				newEmloyee["email"] = trimStrAll(fields[idxField]);
				break;
			}
		  }
		  if (newEmloyee["name"] != "") {
			for (let idxEmployee=0; idxEmployee<employees.length; idxEmployee++) {
				if (employees[idxEmployee]["name"] == newEmloyee["name"]) {
					employees[idxEmployee]["phone"] = newEmloyee["phone"];
					employees[idxEmployee]["email"] = newEmloyee["email"];
					break;
				}
			}				
		  }
...
function loadAll() {
	...
	document.getElementById("employeesDiv").className = "calendarBoxOuter p-3";	
	loadEmployeesFile(fileEmpl, fileDict); 
}

function getRequest(url, actionParams=null) {
	return sendRequest({"actionType": "internal", "actionName": url, "actionParams": actionParams})	
	.then(function(jsonResponse) {
		return jsonResponse;		
	});			
}
